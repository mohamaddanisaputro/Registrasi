				...
				<div class="container">
					<div class="navbar-top">
							...
							<li class="active first-list"><a href="halamanutama">Home</a></li>
								...
								<li><a href="javascript:void(0)" data-toggle="modal" data-target="#exampleModal">Search</a></li>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Cari Data Registrasi</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<div class="modal-body">
					<form action="{{ route('registrasi.caridata') }}" method="post">
						@csrf
						<div class="form-group">
							<label for="daftarevent-option">Nama Event</label>
							<select class="form-control" name="id_event" id="daftarevent_cari">
								@foreach ($daftarevent as $p)
								<option value="{{ $p->id_event }}">{{ $p->nama_event }}</option>
								@endforeach
							</select>
						...
						<div class="form-group">
							<label for="kategorikelas-option">Nama Kategori Kelas</label>
							<select class="form-control" id="id_kategori_event_cari" name="id_kategori_event">
								@foreach ($kategorikelas as $p)
								<option value="{{  $p->id_kategori_event }}">{{ $p->nama_kategori }}</option>
								@endforeach
							</select>
						...
						<div class="form-group">
							<label for="NIK">No identitas</label>
							<input type="number" class="form-control" name="nik" id="nik_cari" placeholder="">
						</div>
